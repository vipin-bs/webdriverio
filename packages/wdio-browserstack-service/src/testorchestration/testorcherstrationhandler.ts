...
const log = logger('wdio-browserstack-service:TestOrchestrationHandler')
    ...
    async reorderTestFiles(testFiles: string[]): Promise<string[] | null> {
        try {
            ...
            if (BrowserstackCLI.getInstance().isRunning()) {
                orderedTestFiles = await BrowserstackCLI.getInstance().test_orchestration_session(testFiles, orchestrationStrategy) || []
            }
            else{
            // SDK CLI flow is not directly applicable in TypeScript, so we'll use the server handler approach
            ...
            this.addToOrderingInstrumentationData("nodeIndex", parseInt(process.env.BROWSERSTACK_NODE_INDEX || "0", 10))
            this.addToOrderingInstrumentationData("totalNodes", parseInt(process.env.BROWSERSTACK_NODE_COUNT || "1", 10))
