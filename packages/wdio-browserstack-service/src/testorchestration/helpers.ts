 ...
 */
function extractGitMetadata(repoPath: string): Record<string, any> {
    try {
        ...
        // Get current branch
        try {
            metadata.branch = execSync('git rev-parse --abbrev-ref HEAD', { cwd }).toString().trim()
        } catch (e) {
            log.debug(`Error getting branch: ${e}`)
        }

        // Get latest commit hash
        try {
            metadata.commitHash = execSync('git rev-parse HEAD', { cwd }).toString().trim()
        } catch (e) {
            log.debug(`Error getting commit hash: ${e}`)
        }

        // Get commit message
        try {
            metadata.commitMessage = execSync('git log -1 --pretty=%B', { cwd }).toString().trim()
        } catch (e) {
            log.debug(`Error getting commit message: ${e}`)
        }

        // Get PR information if available (this varies based on Git hosting)
        try {
            // This logic would need to be adjusted based on your Git provider
            // For GitHub, you might parse PR info from branch name or use GitHub API
            const branchName = metadata.branch
            if (branchName.startsWith('pr/') || branchName.startsWith('pull/')) {
                metadata.isPR = true
                metadata.prNumber = branchName.split('/')[1]
            }
        } catch (e) {
            log.debug(`Error getting PR info: ${e}`)
        }

        // Get changed files
        try {
            // This gets files changed in the last commit
            const changedFiles = execSync('git diff-tree --no-commit-id --name-only -r HEAD', { cwd })
                .toString()
                .trim()
                .split('\n')
                .filter(Boolean)
            ...
            log.debug(`Error getting changed files: ${e}`)
            metadata.changedFiles = []
        }

        return metadata
    } catch (e) {
        log.error(`Error extracting git metadata: ${e}`)
        return {}
    }
}
